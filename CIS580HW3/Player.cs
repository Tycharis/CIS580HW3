...
namespace CIS580HW3
{
    ...
    public class Player
    {
        ...
        public void CheckForGemCollision(IEnumerable<IBoundable> gems)
        {
            Debug.WriteLine($"Checking collisions against {gems.Count()} platforms");
            foreach (IBoundable obj in gems)
            {
                if (obj is Gem && Bounds.CollidesWith(obj.Bounds))
                {
                    Score += 1;
                }
            }
